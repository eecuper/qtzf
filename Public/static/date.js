$(function() {
    ...
    function showDate(year, month) {
        ...
        // console.info('2year'+year+'month'+month);
        $(".year").text(oYear);
        $(".month").text(oMonth);
        ...
        oDate.setDate(1);
        // console.info('oDate.getDay() : '+oDate.getDay());
        ...
        else if (month == 2 && isLeapYear(oYear)) {
            dayNum = 29;
        }
        else {
            dayNum = 28;
        ...
        switch (oDate.getDay()) {
            case 0:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i + 6).text(i + 1);
                    $(aTd).eq(i + 6).attr('para',i + 1);
                }
                break;
            case 1:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i).text(i + 1);
                    $(aTd).eq(i).attr('para',i + 1);
                }
                break;
            case 2:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i + 1).text(i + 1);
                    $(aTd).eq(i + 1).attr('para',i + 1);
                }
                break;
            case 3:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i + 2).text(i + 1);
                    $(aTd).eq(i + 2).attr('para',i + 1);
                }
                break;
            case 4:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i + 3).text(i + 1);
                    $(aTd).eq(i + 3).attr('para',i + 1);
                }
                break;
            case 5:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i + 4).text(i + 1);
                    $(aTd).eq(i + 4).attr('para',i + 1);
                }
                break;
            case 6:
                for (var i = 0; i < dayNum; i++) {
                    $(aTd).eq(i + 5).text(i + 1);
                    $(aTd).eq(i + 5).attr('para',i + 1);
                }
                break;
        }
    }
    ...
    $("#data-con").on("dclick",".abled",function(event){
    ...
    $("#data-con").on("dclick",".disabled",function(event){
        var _index=$(this).attr("data-val");
        var _target=$("#data-con").find("li").eq(_index).find("span").text();
        var aTd = $("#data-con").find('td');
        for (var i = 0; i < $(aTd).length; i++) {
